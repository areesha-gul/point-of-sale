...
export default function CustomerLedger() {
    ...
    const loadLedger = async () => {
        try {
            const response = await customers.getLedger(id);
    ...
    return (
        <div>
            <div className="mb-6">
                <button onClick={() => navigate('/customers')} className="btn-secondary mb-4">
                    ← Back to Customers
                </button>
                <h1 className="text-3xl font-bold">{data.customer.name} - Ledger</h1>
                <p className="text-lg text-gray-600">{data.customer.phone}</p>
